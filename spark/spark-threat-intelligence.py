...
def process_micro_batch(batch_df, args):
    if batch_df.isEmpty():
        return
        
    cast_df = batch_df.withColumn("confidence_score", col("confidence_score").cast(DecimalType(3, 2)))
    cast_df.write \
        .format("iceberg") \
        .mode("append") \
...
def main():
    ...
    # Create streaming query
    data = df.writeStream \
        .foreachBatch(lambda df, id: process_micro_batch(df, args)) \
